...
use toml::de::Error;
use crate::compiler::{HashableMap, QueryTrisult, Text, workshop, wst};
use crate::compiler::cir::{Cir, Struct};
...
fn query_wscript_enum_constant_impl(db: &dyn WorkshopScriptLoader, enum_name: Text, constant_name: Text) -> QueryTrisult<wst::Call> {
    query_wscript_impl(|| db.query_wscript_enum_impl(enum_name).map(|it| it.constants), constant_name)
...
fn query_wscript_event_context_property_impl(db: &dyn WorkshopScriptLoader, enum_name: Text, property_name: Text) -> QueryTrisult<wst::Call> {
    query_wscript_impl(|| db.query_wscript_event_impl(enum_name).map(|it| it.context), property_name)
        .flat_map(|partial_call| partial_call.complete().map_err(|err| CompilerError::PlaceholderError(err)).into())
}
