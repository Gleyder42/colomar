...
pub(super) fn query_wst_rule(db: &dyn Codegen, rule: cir::Rule) -> QueryTrisult<wst::Rule> {
    ...
    let all_decl_args = db
        .query_event_def_by_id(rule.event)
        .map(|event_def| event_def.arguments.into_iter().collect::<HashSet<_>>());

    let supplied_decl_args: HashSet<_> = rule
        .arguments
        .iter()
            ...
            let mut args: Vec<_> = defaulted_args
                ...
                .map(|decl_arg| {
                    (
                        decl_arg.position,
                        decl_arg.name,
                        decl_arg.default_value.expect(ERROR),
                    )
                })
