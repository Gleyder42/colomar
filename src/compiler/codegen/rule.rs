use chumsky::Parser;
use crate::compiler::{cir, QueryTrisult, wst};
...
pub(super) fn query_wst_rule(db: &dyn Codegen, rule: cir::Rule) -> QueryTrisult<wst::Rule> {
    ...
    db.query_event_def_by_id(rule.event)
        .map(|event_def| {
            let supplied_avalue_chains: QueryTrisult<Vec<AValueChain>> = event_def.arguments.iter().enumerate()
                .map(|(index, decl_arg_id)| {
                    ...
                    match (called_arg, decl_arg.default_value) {
                        ...
                        (None, None) => query_error!(CompilerError::MissingArgument { missing_arg: *decl_arg_id, call_site: event_decl.span.clone() })
                    }
                })
                .collect();
        });
