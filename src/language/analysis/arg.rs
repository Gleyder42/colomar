...
pub (in super) fn query_declared_args(db: &dyn DeclQuery, decl_args: Vec<ast::DeclaredArgument>) -> QueryResult<Vec<im::DeclaredArgumentId>, AnalysisError> {
    decl_args.into_iter()
        .map(|decl_arg| db.query_declared_arg(decl_arg))
        .collect::<QueryResult<Vec<_>, _>>()
}

pub (in super) fn query_declared_arg(db: &dyn DeclQuery, decl_arg: ast::DeclaredArgument) -> QueryResult<im::DeclaredArgumentId, AnalysisError> {
